...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='AOGMF',
                        help='the test rgb images root')
    parser.add_argument('--save_path', type=str,
                        default='./output_ablation/')
    parser.add_argument('--train_root', dest='train_root_arg', type=str, default=None,
                        help='optional training dataset root')
    parser.add_argument('--val_root', dest='val_root_arg', type=str, default=None,
                        help='optional validation dataset root')
    ...
    if opt.data_name == 'LEVIR':
        opt.train_root = 'dataset/LEVIR256/train/'
        opt.val_root = 'dataset/LEVIR256/val/'
    elif opt.data_name == 'WHU':
        opt.train_root = 'dataset/WHU-CD256-HANet/train/'
        opt.val_root = 'dataset/WHU-CD256-HANet/val/'
        ...
        opt.train_root = 'dataset/SYSU-CD_256/train/'
        opt.val_root = 'dataset/SYSU-CD_256/val/'
    elif opt.data_name == 'S2Looking':
        opt.train_root = 'dataset/S2Looking-CD256/train/'
        opt.val_root = 'dataset/S2Looking-CD256/test/'

    if opt.train_root_arg is not None:
        opt.train_root = opt.train_root_arg
    if opt.val_root_arg is not None:
        opt.val_root = opt.val_root_arg
    ...
    if opt.model_name == 'HCGMNet':
        ...
        model = AOGMF().cuda()
        #model_weight = torch.load("output/LEVIR/AOGMF_best_iou.pth")
